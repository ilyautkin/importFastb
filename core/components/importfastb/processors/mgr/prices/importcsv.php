        ...
        if ($_POST['parsed']) {
            ...
            $data = array_slice($this->modx->cacheManager->get($key), 0, $limit);
      			...
      			foreach($data as $k => $row) {
      				if ($row[0] && $row[1]) {
      				    if (!$word = $this->modx->getObject('customExtraItem', array('name' => trim($row[0]), 'string1' => trim($row[1])))) {
      				      $word = $this->modx->newObject('customExtraItem', array('name' => trim($row[0]), 'string1' => trim($row[1])));
      				    }
      				    $word->set('string2', trim($row[3]));
      				    $word->save();
      				    if ($row[2]) {
        				    if (!$category = $this->modx->getObject('customExtraOrder', array('name' => trim($row[2])))) {
        				      $category = $this->modx->newObject('customExtraOrder', array('name' => trim($row[2])));
        				      $category->save();
        				    }
