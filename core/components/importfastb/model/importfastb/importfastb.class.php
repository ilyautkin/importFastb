...
/**
 * The base class for importFastb.
 */
class importFastb {
	/* @var modX $modx */
	public $modx;
	...
	function __construct(modX &$modx, array $config = array()) {
		...
		$corePath = $this->modx->getOption('importfastb_core_path', $config, $this->modx->getOption('core_path') . 'components/importfastb/');
		$assetsUrl = $this->modx->getOption('importfastb_assets_url', $config, $this->modx->getOption('assets_url') . 'components/importfastb/');
		...
		$this->modx->addPackage('importfastb', $this->config['modelPath']);
		$this->modx->lexicon->load('importfastb:default');
	}
	...
	public function loadManagerFiles(modManagerController $controller, modResource $resource) {
		...
		$properties = $resource->getProperties('importfastb');

		$controller->addLexiconTopic('importfastb:default');
		$controller->addJavascript($jsUrl . 'importfastb.js');
		...
		<script type="text/javascript">
			MODx.modx23 = ' . $modx23 . ';
			importFastb.config = ' . $this->modx->toJSON($this->config) . ';
			importFastb.config.resID = ' . $resource->id . ';
            importFastb.config.connector_url = "' . $this->config['connectorUrl'] . '";
		</script>', true);
